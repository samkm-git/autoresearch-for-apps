...
func (s *Server) handleGitPush(w http.ResponseWriter, r *http.Request) {
	...
	// Index each new commit and its ancestors recursively
	var indexed []string
	for _, hash := range hashes {
		curr := hash
		for curr != "" {
			parentHash, message, err := s.repo.GetCommitInfo(curr)
			if err != nil {
				break // commit might not exist in this bundle
			}

			// Index if not already in DB
			if existing, _ := s.db.GetCommit(curr); existing == nil {
				// Validate parent exists (unless root)
				if parentHash != "" && !s.repo.CommitExists(parentHash) {
					break 
				}

				if err := s.db.InsertCommit(curr, parentHash, agent.ID, message); err == nil {
					indexed = append(indexed, curr)
				}
			}
			curr = parentHash
		}
